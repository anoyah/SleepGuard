...
应用不会主动终止进程，也不会永久修改系统电源设置；防止睡眠功能只在开启期间持有临时系统断言。所有诊断基于本机命令输出和本地历史记录。
...
- 刷新间隔、语言和忽略规则保存在 `UserDefaults`。
- 防止睡眠功能通过 IOKit 持有临时显示器和系统睡眠断言，应用退出或用户停止后会释放。
- 复制报告时，报告内容会写入系统剪贴板。
